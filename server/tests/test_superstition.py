...
def test_ward_luck_boosts_luck():
    ...
    result = execute_handler(
        WardLuckHandler(), _ctx(actor), _cmd(character.id, {"ritual": "knock-on-wood"})
    )
...
def test_ward_luck_defaults_to_knock_on_wood():
    ...
    result = execute_handler(WardLuckHandler(), _ctx(actor), _cmd(character.id, {}))
...
def test_ward_luck_stacks_with_held_charm():
    ...
    execute_handler(WardLuckHandler(), _ctx(actor), _cmd(character.id, {"ritual": "knock-on-wood"}))
...
def test_ward_luck_grants_luck_component_when_missing():
    ...
    result = execute_handler(
        WardLuckHandler(), _ctx(actor), _cmd(character.id, {"ritual": "toss-salt"})
    )
...
def test_cross_fingers_is_a_smaller_boost():
    ...
    execute_handler(WardLuckHandler(), _ctx(actor), _cmd(character.id, {"ritual": "cross-fingers"}))
...
def test_rejects_invalid_character():
    ...
    result = execute_handler(WardLuckHandler(), _ctx(actor), _cmd("???", {}))
...
def test_rejects_missing_character():
    actor, _room, _character = _world_with_character()
    result = execute_handler(WardLuckHandler(), _ctx(actor), _cmd("entity_9999", {}))
    assert not result.ok
...
def test_rejects_character_without_a_room():
    ...
    result = execute_handler(WardLuckHandler(), _ctx(actor), _cmd(drifter.id, {}))
    assert not result.ok
...
def test_rejects_unknown_ritual():
    actor, _room, character = _world_with_character()
    result = execute_handler(
        WardLuckHandler(), _ctx(actor), _cmd(character.id, {"ritual": "sacrifice-goat"})
    )
    assert not result.ok
